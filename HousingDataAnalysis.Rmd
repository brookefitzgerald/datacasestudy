...
dfm%>% filter(value>min_val)%>%mutate(value=value/min_val/5)%>%
  ...
  coord_flip()+
  theme_few()+
  labs(x = "Relative Variable Importance", y = "Variable Name")
...
ggplot(data = as.data.frame(cbind("Target" =test.full$SalePrice,"Predicted" = predict.gbm)), aes(x = Target, y = Predicted)) + geom_point() + geom_abline(slope = 1, intercept = 0) + xlim(0,5e05) + ylim(0, 5e05) + labs(title= paste("RMSE: ",RMSE.gbm))
```
