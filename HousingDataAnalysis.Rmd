...
lowest_vars<-fullTrain %>% mutate(NeighborhoodSplit = factor(ifelse(Neighborhood %in% c("Blueste","BrDale","BrkSide","Edwards","IDOTRR","MeadowV","Mitchel","OldTown","Sawyer","SWISU"), 1, 0))) %>%
  mutate(GrLivAreaSplit = factor(ifelse(TotalBsmtSF<1419,1,0))) %>%
  select(NeighborhoodSplit,GrLivAreaSplit,SalePrice)

p1<-ggplot(lowest_vars, aes(GrLivAreaSplit,SalePrice))+
  geom_boxplot()+
  theme_few()
p2<-ggplot(lowest_vars, aes(NeighborhoodSplit,SalePrice))+
  geom_boxplot()+
  theme_few()
...
lr.model.full$xlevels$PoolQC <- union(lr.model.full$xlevels$PoolQC, levels(test.full$PoolQC))
...
p1<-qplot(lr.preds.full,test.full$SalePrice)+theme_few()+geom_abline(slope=1,intercept=0)+labs(x = "Full Linear Model Predictions",y="Full Sale Price",title=paste("RMSE: ",lr.RMSE.full))
...
lr.model.imp$xlevels$PoolQC <- union(lr.model.imp$xlevels$PoolQC, levels(test.imp$PoolQC))
...
p2<-qplot(lr.preds.imp,test.imp$SalePrice[!is.na(lr.preds.imp)])+theme_few()+geom_abline(slope=1,intercept=0)+labs(x = "Imputed Linear Model Predictions",y="Imputed Sale Price",title=paste("RMSE: ",lr.RMSE.imp))
...
p3<-qplot(lr.preds.onehot,test.onehot$SalePrice)+theme_few()+geom_abline(slope=1,intercept=0)+labs(x = "One-hot Linear Model Predictions",y="One-hot Sale Price ", title=paste('RMSE: ',lr.RMSE.onehot))

p4<-qplot(lr.model.onehot$residuals)+theme_few()+labs(x = "Index",y="Residuals",title="Linear Model Residuals")
...
lr.model.onehot.edit <- lm(SalePrice~.,train.onehot%>%select_if(function(col) sum(col,na.rm=T)>15)%>%select(-PoolQCNone), na.action = na.omit)
...
p1<-qplot(lr.preds.onehot.edit,test.onehot$SalePrice)+theme_few()+geom_abline(slope=1,intercept=0)+labs(x = "One-hot Linear Model Predictions",y="One-hot Sale Price ", title=paste('RMSE: ',lr.RMSE.onehot.edit))

p2<-qplot(lr.model.onehot.edit$residuals)+theme_few()+labs(x = "Index",y="Residuals",title="Non-Sparse Linear Model Residuals")
...
ggplot()+
  geom_bar(aes(x=reorder(names(sig_coefficients), -sig_coefficients),sig_coefficients),stat="identity")+
  theme_few()+
  labs(title = "Significant p<0.01 One-Hot Coefficients",y="Linear Model Coefficients", x = "Non-Sparse Variables")+
  coord_flip()
...
```{r subset_selection, message=F}

forward_subset<-regsubsets(SalePrice~.,data = train.onehot%>%select_if(function(col) sum(col,na.rm=T)>15)%>%select(-PoolQCNone),method="forward")
forward<-names(one_hot_encoded_cc)[summary(forward_subset)$which[5,]]

backward_subset <-regsubsets(SalePrice~.,data = train.onehot%>%select_if(function(col) sum(col,na.rm=T)>15)%>%select(-PoolQCNone),method="backward")
backward<-names(one_hot_encoded_cc)[summary(backward_subset)$which[5,]]

data.frame("Forward Selected Variables" = names(train.onehot%>%select_if(function(col) sum(col,na.rm=T)>15)%>%select(-PoolQCNone))[summary(forward_subset)$which[5,]], 
           "Backward Selected Variables" = names(train.onehot%>%select_if(function(col) sum(col,na.rm=T)>15)%>%select(-PoolQCNone))[summary(backward_subset)$which[5,]])
```
...
```{r}
fullTrain_knn1<-knnreg(train.onehot.cc[-306],train.onehot.cc$SalePrice, k=1)
knn.1.preds<-predict(fullTrain_knn1,test.onehot.cc[-306])
knn.1.RMSE<-sqrt(sum((knn.1.preds-test.onehot.cc$SalePrice)^2))/length(test.onehot.cc)
...
p1<-qplot(test.onehot.cc$SalePrice,knn.1.preds)+theme_few()+geom_abline(slope = 1, intercept = 0) + labs(x="Actual Sale Price",y="Predictions, k=1",title=paste("RMSE: ",knn.1.RMSE))
...
qplot(1:50,RMSES)+theme_few()+labs(title = paste("Minimum RMSE at k=",which.min(RMSES)), x="k", y="RMSE")
...
tp1 = ggplot(data = as.data.frame(cbind("SalePrice" = cvTest1$SalePrice,"PredictedSalePrice" = Pred1)), aes(x = SalePrice, y = PredictedSalePrice)) + geom_point() + geom_abline(slope = 1, intercept = 0) + xlim(0,5e05) + ylim(0, 5e05) + labs(title= paste("RMSE: ",rmse1))+theme_few()
tp2 = ggplot(data = as.data.frame(cbind("SalePrice" = cvTest2$SalePrice,"PredictedSalePrice" = Pred2)), aes(x = SalePrice, y = PredictedSalePrice)) + geom_point() + geom_abline(slope = 1, intercept = 0) + xlim(0,5e05) + ylim(0, 5e05) + labs(title= paste("RMSE: ",rmse2))+theme_few()
tp3 = ggplot(data = as.data.frame(cbind("SalePrice" = cvTest3$SalePrice,"PredictedSalePrice" = Pred3)), aes(x = SalePrice, y = PredictedSalePrice)) + geom_point() + geom_abline(slope = 1, intercept = 0) + xlim(0,5e05) + ylim(0, 5e05) + labs(title= paste("RMSE: ",rmse3))+theme_few()
tp4 = ggplot(data = as.data.frame(cbind("SalePrice" = cvTest4$SalePrice,"PredictedSalePrice" = Pred4)), aes(x = SalePrice, y = PredictedSalePrice)) + geom_point() + geom_abline(slope = 1, intercept = 0) + xlim(0,5e05) + ylim(0, 5e05) + labs(title=paste("RMSE: ", rmse4))+theme_few()
tp5 = ggplot(data = as.data.frame(cbind("SalePrice" = cvTest5$SalePrice,"PredictedSalePrice" = Pred5)), aes(x = SalePrice, y = PredictedSalePrice)) + geom_point() + geom_abline(slope = 1, intercept = 0) + xlim(0,5e05) + ylim(0, 5e05) + labs(title=paste("RMSE: ",rmse5))+theme_few()
...
importance=data.frame(Predictor =Imp1[,1],RForest1=Imp1[,2],RForest2=Imp2[,2],RForest3=Imp3[,2],RForest4=Imp4[,2],RForest5=Imp5[,2])
...
dfm%>% filter(value>min_val)%>%mutate(value=value/min_val/5)%>%
  ...
  geom_bar(stat="identity")+
  labs(y = "Relative Variable Importance", x = "Variable Name")+
  coord_flip()+
  theme_few()
...
Due to the fact that in our random forest model we have overestimated low sale prices and underestimated high sale prices, we can address this bias by applying some boosting techniques (such as the Gradient Boosting Model).
As we can observe in the figure below, the GBM model was more accurate in our prediction than the random forests model since it is reduced the bias when overestimating sale prices that are low. However, with this method we did not manage to reduce the bias since it stills underestimate sale prices that are high.
...
RMSE.gbm = round(sqrt(sum((predict.gbm-test.full$SalePrice)^2))/length(predict.gbm),2)  

ggplot(data = as.data.frame(cbind("SalePrice" =test.full$SalePrice,"PredictedSalePrice" = predict.gbm)), aes(x = SalePrice, y = PredictedSalePrice)) + geom_point() + geom_abline(slope = 1, intercept = 0) + xlim(0,5e05) + ylim(0, 5e05) + labs(title= paste("RMSE: ",RMSE.gbm))+theme_few()
```
...
```{r}
data.frame(Method = c("Average Decision Tree ",
                      "Linear Model with One-Hot Encoded Data",
                      "Non-Sparse Linear Model with One-Hot Encoded Data",
                      "KNN Regression",
                      "Average Random Forest",
                      "Boosting"),
           RMSE = c(tree.RMSE.avg,
                    lr.RMSE.onehot,
                    lr.RMSE.onehot.edit,
                    min(RMSES),
                    mean(rmse1,rmse2,rmse3,rmse4,rmse5),
                    RMSE.gbm)) %>%
  ggplot(aes(x=reorder(Method,-RMSE),y=RMSE))+
    geom_bar(stat="identity")+
    theme_few()+
    labs(x="Predictive Method",title="Method Comparison")+
    coord_flip()
```
